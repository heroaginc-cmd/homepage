    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#1A1A1A]">
      <nav className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8">
          <li><Link href="/about" className="text-sm text-white hover:text-[#F18E24] transition-colors">会社概要</Link></li>
          <li><Link href="/services" className="text-sm text-white hover:text-[#F18E24] transition-colors">サービス</Link></li>
          <li><Link href="/works" className="text-sm text-white hover:text-[#F18E24] transition-colors">実績</Link></li>
          <li><Link href="/blog" className="text-sm text-white hover:text-[#F18E24] transition-colors">ブログ</Link></li>
        </ul>
        <div className="hidden md:flex items-center gap-4">
          <a href="https://lin.ee/vBEfQwi" target="_blank" rel="noopener noreferrer" className="bg-[#F18E24] text-white text-sm px-5 py-2 hover:bg-[#D4780F] transition-colors">
            LINEで相談する
          </a>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-[#1A1A1A] border-t border-[#404040] px-6 py-4">
          <ul className="flex flex-col gap-4">
            <li><Link href="/about" className="text-sm text-white" onClick={() => setIsOpen(false)}>会社概要</Link></li>
            <li><Link href="/services" className="text-sm text-white" onClick={() => setIsOpen(false)}>サービス</Link></li>
            <li><Link href="/works" className="text-sm text-white" onClick={() => setIsOpen(false)}>実績</Link></li>
